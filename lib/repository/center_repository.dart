...
class CenterRepository{
  ...
  static final CenterRepository centerRepository=new CenterRepository._internal();
 ...
 // HashMap<int,CarStateVM> carStateVMMap=new HashMap();
  ...
  List<CurrentUserAccessableActionModel> currentUserAccessableActionModel=new List();
 ...
 fetchGPSStatus() async {
  ...
  Color getBackNavThemeColor(bool dark){
   Color res= !dark ?  Colors.white : Colors.black87;
   return res;
  }
 ...
 static int getCurrentCarId(){
   ...
   setInitCarStateVMMap(int carCounts)
   {
     for(var carIndex=0;carIndex<carCounts;carIndex++) {
       ...
       CarStateVM carStateVM = new CarStateVM(
         ...
         isCaputOpen: false,
         carIndex: carIndex,
         color: getCurrentCarColor(
             Constants.colorIdToColorsMap[colorId], carIndex),
         bothClosed: true,
       );
  ...
  initCarColorsMap()
  {
    if(carColorsMap==null)
      carColorsMap=new HashMap();
    ...
    carsColorMap.putIfAbsent(mat.MaterialColor.GREY, () => colors[5]);
  ...
  {
    if(userModelInCach==null)
      userModelInCach=new SaveUserModel();
  ...
  mat.MaterialColor getCurrentCarColor(Color color,int value)
  {
    ...
    {
      colr=carColorsMap[Constants.colorIdToColorsMap[value].value /*colors[value].value*/];
  ...
  setCurrentUserAccessableActionModel(List<CurrentUserAccessableActionModel> actions){
  ...
  setCarState(CarStateVM stateVM)
  {
    if(carStates==null)
      carStates=new List();
   List<CarStateVM> result= carStates.where((s)=> s.carIndex==stateVM.carIndex).toList();
   if(result==null || result.length==0)
  ...
  setUserLoginedInfoCached(UserLoginedInfo data)
  {
    if(this.cachedUserInfo==null)
      cachedUserInfo=new UserLoginedInfo();
      cachedUserInfo=data;
  }
  ...
  CarStateVM getCarState(int indx,CarStateVM stateVM)
  {
    ...
    List<CarStateVM> result= carStates.where((s)=> s.carIndex==carIndex ).toList();
    if(result!=null && result.length>0)
      return result[0];
    return null;
  }
  ...
  setDeviceModels(List<DeviceModel> devices){
    if(carDeviceModels==null)
      carDeviceModels=new List();
  ...
  {
    if(carsToAdmin==null)
      carsToAdmin=new List();
    if(carsToAdmin!=null &&
        carsToAdmin.length>0)
        carsToAdmin.clear();
  ...
  {
    if(actionToRoles==null)
      actionToRoles=new List();
  ...
  ActionModel getActionByActionCode(int actionId)
  {
    if(actions!=null &&
      actions.length>0) {
      List<ActionModel> result = actions.where((a) => a.ActionId == actionId).toList();
      if (result!=null && result.length>0)
        return result.first;
      else
        return null;
  ...
  {
    if(this.carsToAdmin!=null &&
    this.carsToAdmin.length>0)
      {
  ...
  showSnackLogin(GlobalKey<ScaffoldState> scaffoldKey,BuildContext context,String message,bool isLoading)
  {
 ...
 String showFilterDate(BuildContext context, bool from) {
    ...
    dtpicker.DatePicker.showDatePicker(context,
        ...
        onChanged: (date) {
            ...
            String resultDate=newDate.year.toString()+'/'+newDate.month.toString()+'/'+newDate.day.toString();
              return resultDate;
  ...
  Future<InitDataVM> loadInitData(bool hasInternet) async {
    ...
    if(hasInternet) {
      ...
      try {
        List<SaveUserModel> userInfo = await restDS.getUserInfo(
            userId.toString());
        if (userInfo != null && userInfo.length > 0)
          if (userInfo[0].isSuccess)
      ...
      catch(error)
    {
      ...
      if(actionToRoles!=null &&
          actionToRoles.length>0)
        centerRepository.setActionToRoles(actionToRoles);

      List<CurrentUserAccessableActionModel> accessableActionModel=new List();
      accessableActionModel=await restDS.getGetCurrentUserAccessableActions(userId);
      if(accessableActionModel!=null &&
          accessableActionModel.length>0)
        centerRepository.setCurrentUserAccessableActionModel(accessableActionModel);
    ...
    else
    {
      ...
      centerRepository.setCurrentUserAccessableActionModel(FakeServer.createCurrentUserAccessableActionModel());
