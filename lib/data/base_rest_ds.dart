...
abstract class BaseRestDS<T> extends RestDatasource {
  ...
  static final GET_PLAN_BY_PLANID_URL = BASE_URL + BASE_PLAN_URL+ "/GetByPlanId";
  ...
  static final GET_ADMIN_CARS_BY_ID_URL= BASE_URL+BASE_ADMIN_CAR_URL+ "/GetAdminCarByUserId";
  ...
  Future<ServiceResult> login(String username, String password) async{
    ...
    try {
      ...
      return netUtil.getUri(uri, theaders: getHeaders()).then(( res) {
  ...
  Future<User> authenticate(String username, String password) async{
    return netUtil.post<User>('AUTHENTICATE_URL', body: {
      "username": username,
      "password": password
    }).then((User res) {
      //print(res.toString());
  ...
  Future<SaveMagicarResponeQuery> register({Map<String,dynamic> body} ) async{
    ...
    return netUtil.post<dynamic>(SAVE_USER_MAGICAR_URL, body: body,headers: headers
    ).then((dynamic res) {
