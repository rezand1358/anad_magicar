...
class _CarCardState extends State<_CarCard> with TickerProviderStateMixin {
  ...
  var _authData = {'pelak': '', 'tip': '','modelId': '', 'colorId': '','distance': '','brandId': '','cancel':''};
 ...
 var pelak_part2_title=[{'title': 'الف'},
  ...
  List<CarModel> fetchCarModels(List<CarModel> models)  {
    if(cmodels!=null && cmodels.length>0)
      cmodels.clear();
    cmodels=List.from(centerRepository.getCarModels().where((cm) => cm.brandId==brandId).toSet().toList());
    if(cmodels!=null && cmodels.length>0)
  ...
  List<CarModelDetail> fetchCarModelDetails(List<CarModelDetail> models)  {
      if(modelDetails!=null && modelDetails.length>0)
        modelDetails.clear();
      modelDetails = List.from(centerRepository.getCarModelDetails().where((cm) => cm.carModelId==modelId).toSet().toList());
      if(modelDetails!=null && modelDetails.length>0)
        ...
        builder: (FormFieldState<ApiCarColor> field) {
          return InputDecorator(
            decoration: InputDecoration(
              ...
              errorText: field.errorText,
              contentPadding:
              EdgeInsets.only(top: 10.0, bottom: 0.0,right: 10.0,left: 10.0),
              border: InputBorder.none,
            ),
  ...
  Widget _buildCarModelField(double width, LoginMessages messages,int brandId) {
