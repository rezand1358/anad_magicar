...
import 'package:anad_magicar/data/fetch_app_start_data.dart';
...
class LoadingScreen extends StatefulWidget {
  ...
  LoadingScreen({Key key,this.messageHandler}) : super(key: key);
...
_controller = new AnimationController(
  ...
  Widget _buildBody() {
    if (_loadingInProgress) {
      ...
      children: [
        new Center(
          child: new SpinKitCubeGrid(color: Colors.white,size: 80.0,)
      ),
      new Center(
...
Widget _buildAnimation() {
 ...
 Future<InitDataVM> _loadData() async {
   ...
   if(hasInternet) {
     ...
     if (roles != null)
       centerRepository.setRole(roles);
     ...
     carModelDetails = await carModelDetaillDS.getAll(BaseRestDS.GET_CAR_MODEL_DETAIL_URL);
     ...
     if(invoices!=null)
       centerRepository.setInvoices(invoices);
